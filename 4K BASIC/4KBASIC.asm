; ZX80 4K BASIC for Minstrel 4th

; Building:
; TASM.EXE -t80 -fff -b 4KBASIC.asm 4KBASIC.bin 

; Original ZX80 4K BASIC ROM disassembly and comments by Geoff Wearmouth

; Mods Dave Curran 2023, updated 2025-05-17

; ZX80 4K BASIC for Minstrel 4th memory map:
;
; 0000 - 0DFF   Original BASIC ROM with minimal modifications
; 0E00 - 0FFF   ZX80 font
; 1000 - 1FFF   New display, load and save code

; 2000 - 23FF   1K Video RAM mirror
; 2400 - 27FF   1K Video RAM 40x25 characters

; 2800 - 2BFF   1K Font RAM mirror (write only)
; 2C00 - 2FFF   1K Font RAM 128 characters (write only)

; 3C00 - 3FFF   1K RAM (available, not used by BASIC)
; 4000 - 7FFF   16K RAM
; 8000 - FFFF   32K RAM (available, not used by BASIC)
...
;; START
L0000:  JP      INIT_4TH         ; Jump to 4TH init code, which will then go to RAM-FILL
...
; --------------------------------
; Called once every frame does nothing other than wake from halt

;; MASK-INT
L0038:  RET

;; Continuation of CH_ADD relocated here to avoid blocking NMI
L0039:
        ...
        SET     2,(IY+$01)      ; update FLAGS set K mode.
...
L013C:  CALL    L01AD           ;; routine DISP-2
                                ;# this second call generated the bottom border
                                ;# (or in the case of the 4TH, waits for the VSync)
...
;# Bits 6 and 7 of the port FE are not implemented on the 4TH
;# The bus pullup resistors will make this read as a 1, UK 50Hz.
...
L0194:  DJNZ    L0194           ;; (13,8) wait a while looping to KB-4 
        ...
        CALL    L01AD           ;; routine DISP-2 
                                ;# this call will generate the top border
                                ;# and visible screen
...
;# This is the entry point used by the flicker free games

L01B0:  JP    DISPLAY_ROUTINE   ;# jump to the new routine
      
;##############
; SPACE

        DEFB    $FF, $FF, $FF   ;# three spare bytes
...
L074D:  LD      BC,$1721        ; set line to 23 and column to 33.
...
L07AC:  DEFB    $00             ; Class-00 - no further operands.
        ...
        DEFW    NEW_LOAD        ; address of new LOAD function

;; P-SAVE
L07AF:  DEFB    $00             ; Class-00 - no further operands.
        ;DEFW    L01B6           ; address: $01B6
        DEFW    NEW_SAVE        ; new save function
...
L093D:  CALL    L0934           ; routine GOTO sets up any supplied line number.
        ;JP      MISTS_OF_TIME   ; put up mists of time, then jump to clear 
...
;
; Clear the video RAM and load the ZX80 font into the font RAM
; Display a banner, which will remain until interrupts are enabled
;

INIT_4TH:

; Fill all the video RAM with zeroes (space)
CLEAR_VIDEO_RAM:
        LD      HL, $2400       ; start of video RAM
        LD      DE, $2401       ; the next byte
        LD      BC, $03FF       ; number of bytes to clear (video RAM + mirror of font RAM)
        LD      (HL), $00       ; Clear the first byte
        LDIR                    ; Clear all the rest

; copy the ZX80 font into the first half of font RAM
COPYFONT:        
        LD      BC, $0200       ; There are 64 bit-mapped 8x8 characters
        LD      DE, $2C00       ; Destination is write only font RAM
        LD      HL, $0E00       ; ZX80 font in ROM
        LDIR                    ; Copy font from ROM to RAM

; copy a banner message onto the top of the screen
BANNER:
        LD      DE, $2400       ; Copy banner text to top of 4TH video RAM 
        LD      HL, BANNER_TEXT
        LD      BC, BANNER_TEXT_END-BANNER_TEXT
        LDIR

; clear the remaining 64 characters in the font RAM
CLEARUDGS:
        LD      HL, $2E00       ; start of second 64 character font RAM
        LD      DE, $2E01       ; the next byte
        LD      BC, $01FF       ; number of bytes to clear (video RAM + mirror of font RAM)
        LD      (HL), $00       ; Clear the first byte
        LDIR                    ; Clear all the rest
...
ON_TO_RAM_CHECK:
        ...
        JP      L0261           ; on to RAM-FILL.


; interrupting the init routine to add a banner
INIT_BANNER:
        LD      ($400A),HL      ; set the system variable E_LINE.
        LD      (HL),$B0        ; insert the character inverse 'K'.
        ...
        ; replacement CLS function
        LD      (HL),$76        ; insert a single newline.
        INC     HL              ; advance address.

        EX      DE,HL           ; DE now contains D-FILE location
        LD      HL, BANNER_TEXT
        LD      BC, BANNER_TEXT_END-BANNER_TEXT
        LDIR

        EX      DE,HL           ; HL now contains D-FILE location
        
        CALL    L074D           ; call the original CLS routine

        JP      L0296           ; return to just after the CLS


;##########################################################
; Double buffer the ZX80 video frame to the 4TH screen RAM

; this is called twice, once to produce the top border and main screen
; and again to produce the bottom border.

; The first call witll copy the screen, 
; the sender will wait for the interrupt at the end of the screen

DISPLAY_ROUTINE:
        DEC     B               ; decrement row counter (24 = top, 1 = bottom)
        JR      NZ, COPY_SCREEN ; if B was > 1, draw the screen to the 4TH RAM

; second call, display has finished, so wait until the start of the next frame                                
        EI                      ; enable interrupts.
        HALT                    ; halt and wait for the next interrupt
        DI                      ; disable interrupts until next time.

        RET                     ; finished.

; On the first call, copy the display to the 4TH video RAM
COPY_SCREEN:
        RES     7, H            ; move the display file pointer back to low RAM

        INC     HL              ; Skip over the first HALT / newline
        LD      DE,$2400        ; Minstrel 4th screen RAM
                
        LD      C, B            ; line counter from B (pad to 24 at end?)
...
COPY_CHAR:
        ...
        CP      $76             ; check for CR/Halt
        JR      Z, PAD_LINE     ; skip to padding if it is

        LD      (DE),A          ; write the character
        INC     DE              ; next character
        DJNZ    COPY_CHAR       ; loop
        INC     HL              ; skip next character (should be HALT / newline)
        JR      CHECK_END       ; finished?

PAD_LINE:
        XOR     A               ; set A to $00, space        
PAD:        
        LD      (DE),A          ; write a space
        INC     DE
        DJNZ    PAD             ; loop until the end of the line

CHECK_END:
        DEC     C               ; end of the screen?
        JR      NZ, COPY_LINE

FILL_TO_END:
        LD      A, $27          ; check if we are at the 
        CP      D               ; is DE > $27xx
        JP      C, DISPLAY_END  ; yes
        XOR     A               ; no, fill with space
        LD      (DE), A         ; 
        INC     DE              ;
        JR      FILL_TO_END     ; back for more

DISPLAY_END:
                                ; B is 0 at this point
        RET                     ; finished
...
DRAW_BOX:
        ...
        LD      HL, $2400       ; start of video RAM
        LD      DE, $2401       ; the next byte
        LD      BC, $03FF       ; number of bytes to clear
...
NEW_LOAD:       
        POP     BC              ; discard the return address.
...
;; LOAD-1
LOAD_1: LD      BC,$5712        ; set the timing constants.
...
;; LOAD-2
LOAD_2: LD      D, A            ; save the old state 
        
        RRA                     ; test the outer key.
        JP      NC,L0203        ; back, if pressed, indirectly to MAIN-EXEC
        RLA                     ; rotate back
        
;; LOAD-3
LOAD_3: LD      A,$7F           ; read from port $7FFE.
        IN      A,($FE)         ; the keyboard row with space.
        AND     IO_FE_MASK      ; mask all but the tape and space column bits

        CP      D               ; test if this is the same as the previous byte
        JP      NZ, LOAD_2      ; if different go and test for break or restart counter        
        
        DEC     BC              ; decrement timer
        LD      A,B             ; and test 
        OR      C               ; for zero.
        JR      NZ,LOAD_3       ; back if not to inner loop LOAD-3.

        ; at this point, the value has been stable for a few seconds
...
LOAD_WAIT:
        ...
        JP      Z,L024D         ; yes space / break pressed -> LD-ABORT
...
LOAD_SAME:
        ...
        ; no transition has been detected for 16 tests

        LD      A, C            ; check the transition count
        
        CP      $04             ; is it < 4
        JR      C, LOAD_BYTE    ; error, start again
        ...
        JR      LOAD_BYTE       ; error, start again
...
LOAD_TEST_END:                  ; replacement for L01F8
        INC     HL              ; increase pointer.
        PUSH    HL              ; store HL
        ...
        SCF                     ; set carry
        SBC     HL,DE           ; trial subtraction.
        JR      C,LOAD_END      ; end if no more bytes to do.

        CALL    DISPLAY_HL      ; show the progress

        POP     HL              ; restore pointer.
        LD      D, B            ; restore D

        JR      LOAD_BYTE       ; get next byte

LOAD_END:
        ; end has been reached
        
        ; check for nocash LD H,L trick by Martin Korth
        LD      DE,$7665        ; check if (402A) = 7665
        LD      HL,($402A)      ; 
        CCF                     ; clear carry
        SBC     HL,DE           ; check if they are the same
        JP      Z,$4040         ; if they match, jump to $4040

                                ; not an autostart
        JP      L0283           ; JUMP to MAIN-EXEC to list the program
...
;; SAVE
        ...
        XOR     A               ; start with the output low
        OUT     ($FE), A        ;

;; SAVE-1
SAVE_1: LD      A,$7F           ; read port $7FFE.
        IN      A,($FE)         ; all 16 bits are placed on address bus.
        RRA                     ; test for the space key.
        JR      NC,SAVE_EXIT    ; forward, if pressed, indirectly to MAIN-EXEC.

;; SAVE-2
SAVE_2: DJNZ    SAVE_2          ; delay self-looping to SAVE-2

        DEC     DE              ; decrement
        LD      A,D             ; and test
        OR      E               ; for zero.
        JR      NZ,SAVE_1       ; back if not zero to outer delay loop SAVE-1.

        LD      HL,$4000        ; commence saving at start of RAM.

;; SAVE-3
SAVE_3: LD      DE,$F808        ; register E counts the 8 bits.
                                ; $F8 is first delay.

;; EACH-BIT
EACH_BIT:  
        RLC     (HL)            ; spin the actual program byte.
        SBC     A,A             ; $FF or $00.
        AND     $05             ; $05 or $00.
        ADD     A,$04           ; $09 or $04.
        LD      C,A             ; timer to C.
...
;; SAVE-4
SAVE_4: LD      A, $08          ; set save output high
        OUT     ($FE),A         ; pulses

        LD      B,$24           ; delay counter.

;; SAVE-5
SAVE_5: DJNZ    SAVE_5          ; self loop for delay to SAVE-5
...
;; SAVE-6
SAVE_6: DJNZ    SAVE_6          ; self loop for delay2 to SAVE-6

        DEC     C               ; decrement pulse counter
        JR      NZ,SAVE_4       ; back while more to SAVE-4.

        LD      B,D             ; a terminating delay - D is zero (256).

;; SAVE-7
SAVE_7: NOP                     ; 4 T-states.
        DJNZ    SAVE_7          ; execute the NOP 256 times.

        LD      D,$FE           ; subsequent timing value
        DEC     E               ; decrement the 8 counter.
        JR      NZ,EACH_BIT     ; back if more to EACH-BIT.

        LD      A,$7F           ; read the space row 
        IN      A,($FE)         ; 
        RRA                     ; test for space key pressed
        JR      NC,SAVE_EXIT    ; forward, if so, indirectly to MAIN-EXEC.

SAVE_TEST_END:                  ; replacement for L01F8
        INC     HL              ; increase pointer.
        PUSH    HL              ; store HL
        EX      DE,HL           ; move the pointer value to DE
        LD      HL,($400A)      ; load HL with E_LINE - the location following
        ...
        SCF                     ; set carry
        SBC     HL,DE           ; trial subtraction.
        JR      C,SAVE_EXIT     ; Finished, exit here

        CALL    DISPLAY_HL      ; show the progress

        POP     HL              ; restore pointer.

        JR      SAVE_3          ; else back to do another byte.
...
DIGITS_POS      EQU     $2572           ; LINES2_POS+9
...
BOXTOP:
        DEFB    $87, $83, $83, $83, $83, $83, $83, $83, $83, $83, $83, $83, $83, $86

LOADING:
        DEFB    $02, _L, _O, _A, _D, _I, _N, _G, __, _DOT, _DOT, _DOT, _DOT, $82 

SAVING:
        DEFB    $02, _S, _A, _V, _I, _N, _G, __, __, _DOT, _DOT, _DOT, _DOT, $82 

BOXBOTTOM:
        DEFB    $85, $03, $03, $03, $03, $03, $03, $03, $03, $03, $03, $03, $03, $84

;##########################################################

BANNER_TEXT:
        ; "ZX80 BASIC FOR MINSTREL 4TH V1.0"
        DEFB    _Z, _X, _8, _0, __, _B, _A, _S, _I, _C, __
        DEFB    _F, _O, _R, __, _M, _I, _N, _S, _T, _R, _E, _L, __
        DEFB    _4, _T, _H, __, _V, _1, _DOT, _0
BANNER_TEXT_END:
